...
export function extractEndpointsFromScan(scan = {}) {
  const out = [];
  (scan.endpoints || []).forEach((e) => e?.url && out.push({ url: e.url, method: e.method || 'GET', contentType: e.contentType || '' }));
  (scan.browser?.apiCandidates || []).forEach((c) => c?.url && out.push({ url: c.url, method: c.method || 'GET', contentType: c.contentType || '' }));
  (scan.browser?.deepLinks || []).forEach((l) => l?.href && out.push({ url: l.href, method: 'GET', contentType: '' }));
  return out;
}
...
export function buildUrlWithParams(BASE_URL, opts) {
  const {
